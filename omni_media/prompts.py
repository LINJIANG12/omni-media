...
2. 【绝对禁止二次总结与概述】：严禁生成任何“本文总结了”、“主要内容概括如下”、要点提炼、章节大纲或导言后记！必须且仅输出原音频逐字对应的纯净文本。
3. 【纯净正文输出】：严禁以第一人称对转录过程进行元叙述（例如“好的，我来为您转录”、“开始转录：”等废话）。
4. 【去除口语碎词】：平滑去除“嗯、啊、这个、那个、大家懂吧”等无意义口头禅与开头设备调试闲聊。
5. 【自然分段】：根据语义与话题转换自然换行分段，保持行文连贯清晰；无需标注时间戳，严禁臆造或猜测虚假时间戳。
6. 【代码与数学公式】：原音频中涉及的代码请使用 ```语言 代码块 完整包裹；公式请使用 LaTeX 规范 ($...$ 或 $$...$$)。
"""
