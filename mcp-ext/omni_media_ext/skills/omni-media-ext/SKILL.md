...
- **返回内容会被校验**：外部模型偶尔把**自己的写作提纲**当结果返回（HTTP 200、内容非空）。
  `mode="transcribe"` 命中即判为无效，会在 `max_retries` 内自动重读，仍失败则报错——
  **不会把提纲静默当成逐字稿**，你按报错重试本片或调小 `duration_minutes` 即可。
- **报错指向「上游」时不要去改端点配置**：网关自身可达（`status --probe` 只发
  `GET /models`，因此会报「可达」），失败发生在上游鉴权/配额或网络。先修代理与上游再重跑；
  反复重试同一个调用没有意义。
