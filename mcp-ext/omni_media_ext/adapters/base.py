...
class BaseHostAdapter(ABC):
    ...
    def __init__(
        ...
        # 服务自身的配置文件（与「宿主配置文件」是两回事）：给了就写进服务启动参数，
        # 让宿主拉起的进程读指定文件，而不是走默认查找顺序。
    ...
    def is_registered(self) -> bool:
        """Checks if omni-media-ext is currently registered in this host configuration."""
        pass

    @abstractmethod
    def build_applied_config(self, current_data: Dict[str, Any]) -> Dict[str, Any]:
        """Generates new configuration dictionary with omni-media-ext injected."""
        pass

    @abstractmethod
    def build_unapplied_config(self, current_data: Dict[str, Any]) -> Dict[str, Any]:
        """Generates new configuration dictionary with omni-media-ext removed."""
        pass
    ...
    def preview_unapply(self) -> Tuple[bool, str, Dict[str, Any]]:
        """Previews the diff of removing omni-media-ext configuration.
    ...
    def unapply(self) -> Tuple[bool, str]:
        """Removes omni-media-ext entry from host configuration."""
        if not self.is_registered():
            return True, f"{self.display_name} 中未检测到 omni-media-ext 配置，跳过。"
    ...
    def check_status(self) -> Dict[str, Any]:
        ...
        if registered:
            ...
            status = "INFO"
            msg = f"配置文件存在，但未挂载 omni-media-ext ({path})"
        else:
            status = "INFO"
